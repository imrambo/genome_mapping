...
#     return fastq_dict
#=============================================================================
def find_fastq_pairs(fastq_list, nheader, exclude = False):
    '''
    ...
    for fastq in fastq_list:
        ...
        if magic.from_file(fastq).startswith('gzip compressed data') and fastq.endswith('.gz'):
            with gzip.open(fastq, 'r') as fq:
                head_list = [l.decode('utf-8').split() for l in islice(fq, nheader) if re.match(r'^\@.*?\:\d+\:.*?\:\d+\:\d+\:\d+\:\d+\s+\d\:.*?\:[ACTGN]+', l.decode('utf-8'))]
        else:
            with open(fastq, 'r')as fq:
                #head_list = [l.split() for l in islice(fq, nheader) if l.startswith('@')]
                head_list = [l.split() for l in islice(fq, nheader) if re.match(r'^\@.*?\:\d+\:.*?\:\d+\:\d+\:\d+\:\d+\s+\d\:.*?\:[ACTGN]+', l)]
